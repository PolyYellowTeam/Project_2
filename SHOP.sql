USE MASTER
GO
IF EXISTS(SELECT * FROM SYSDATABASES WHERE NAME='SHOP') DROP DATABASE SHOP
GO
...
------------------------------------------PICTURES
IF  EXISTS (SELECT * FROM SYSOBJECTS  WHERE name = 'Pictures')	DROP TABLE Pictures
GO
CREATE TABLE Pictures(
Picture_id INT IDENTITY Not null,
Product_id varchar(10) not null,
Picture_link varchar(200) Not null,
CONSTRAINT PK_Pic PRIMARY KEY (Picture_id),
CONSTRAINT FK_Pic FOREIGN KEY (Product_id) REFERENCES Products (Product_id)
)
------------------------------------------PAYMENT_METHOD
