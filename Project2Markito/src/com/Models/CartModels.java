	...
	public boolean checkOut(String userName,List<Products> productList) {
		...
		try {
			...
			for (Products obj : productList) {
				double totalPrice = (Double.parseDouble(obj.getPrice().toString().trim())*(double)(100-obj.getDiscount())/100)*obj.getQuantity();
			...
			sql = "";
			for (Products obj : productList) {
				sql = sql + String.format("INSERT INTO Cart_Details (Cart_id,Product_id,Quantity,Price) VALUES ("+cartId+",'"+obj.getProductId()+"',"+obj.getQuantity()+","+obj.getPrice()+");");
			}
			session.createSQLQuery(sql).executeUpdate();
			session.flush();
			session.clear();
